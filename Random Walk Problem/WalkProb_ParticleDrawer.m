...
k = 100;
max_trials = 100000 * k;
take_a_picture_every_blank_frames = 1;
rate_of_growth = 0.001;
rate_cap = 1000;
...
for time = 1:time_limit
    ...
    i_vec(time) = i;
    j_vec(time) = j;
    P(k+1-i,j+1) = P(k+1-i,j+1) + 5;

    if mod((trial - 1) * time_limit + time,frame_instant) == 0
...
% for is = 1:length(i_vec)-1
%     for js = 1:length(j_vec)-1
%         P(k+1-i_vec(is),j_vec(js)+1) = 0;
%     end
% end
